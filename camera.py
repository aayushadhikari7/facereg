    ...
    def run(self):
        try:
            ...
            if not self.cap.isOpened():
                self.error_signal.emit(f"Error: Could not open video stream from {self.rtsp_url}. Please check the URL, camera status, and network connectivity.")
...
class ObjectDetectionThread(QThread):
    ...
    MOTION_LOG_COOLDOWN = 2 

    def __init__(self, model_path='yolov8x.pt'): # yolov8x.pt 
        super().__init__()
                    ...
                    if self._previous_gray_frame is None:
                        ...
                        if motion_detected:
                            ...
                            if (current_time - self._last_motion_log_time) > self.MOTION_LOG_COOLDOWN:
                                ...
                                self.new_object_detected_signal.emit(log_entry, 'motion') 
                        ...
                        for box in r.boxes:
                            ...
                            if confidence > 0.4:
                                ...
                                if (class_name not in self.last_logged_time_per_class or
                                    ...
                                    self.new_object_detected_signal.emit(log_entry, 'default') 
...
class CameraFeedApp(QWidget):
    def __init__(self):
        ...
        # For RTSP camera:
        self.rtsp_url = "rtsp://admin:qwertyuiop1@192.168.0.108:554/cam/realmonitor?channel=1&subtype=0"
    ...
    def show_custom_message(self, title, message, is_error=False):
        """Displays a custom styled message dialog."""
